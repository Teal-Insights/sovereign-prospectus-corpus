...
it("casts dates to ISO strings in SQL (Arrow Date32 representation varies)", () => {
  expect(buildListSql(DEFAULTS)).toContain(
...
it("defaults to sovereign scope and drops the filter when included", () => {
  ...
  expect(
    buildListSql({ ...DEFAULTS, includeNonSovereign: true }),
  ).not.toContain("is_sovereign = true");
});
...
it("region filters use the Unknown COALESCE guard", () => {
  ...
  expect(sql).toContain(
    "COALESCE(region, 'Unknown') IN ('Sub-Saharan Africa', 'Unknown')",
  );
});
...
it("status counts arms flip to TRUE/FALSE when a toggle is inactive", () => {
  ...
  expect(allIn).toContain(
    "FILTER (WHERE TRUE AND FALSE)::INTEGER AS hidden_scope",
  );
  expect(allIn).toContain(
    "FILTER (WHERE TRUE AND FALSE)::INTEGER AS hidden_hi",
  );
...
it("whitespace-only or empty q contributes no search clause", () => {
  ...
  expect(buildListSql({ ...DEFAULTS, q: "  \t " })).toBe(
    buildListSql(DEFAULTS),
  );
});
...
it("search clauses AND with explicit filters in the counts WHERE", () => {
  const sql = buildStatusCountsSql({
    ...DEFAULTS,
    countries: ["KEN"],
    q: "bond",
  });
...
function whereClause(sql: string): string {
  ...
  expect(
    match,
    `SQL has no WHERE clause before ORDER BY:\n${sql}`,
  ).not.toBeNull();
  return match![1];
}
...
it("export SQL uses the 10001-row cap and never paginates with an offset", () => {
  ...
  expect(sql).toContain("END AS raw_title");
  expect(sql).not.toContain("issuer_name, title, raw_title,");
});
...
it("export and list SQL emit byte-identical WHERE clauses for countries, q, and toggles", () => {
  ...
  expect(whereClause(buildExportSql(filters))).toBe(
    whereClause(buildListSql(filters)),
  );
});
