...
it('quotes a field containing a comma', () => {
  const { csv } = toCsv([{ ...ROW, title: 'Bonds, Series A' }], 'https://prospectus.example');

  expect(csv).toContain('"Bonds, Series A"');
});

it('quotes a field containing a quote and doubles the embedded quote', () => {
  const { csv } = toCsv([{ ...ROW, title: 'The "Notes"' }], 'https://prospectus.example');

  expect(csv).toContain('"The ""Notes"""');
});

it('quotes a field containing a newline', () => {
  const { csv } = toCsv([{ ...ROW, title: 'First line\nSecond line' }], 'https://prospectus.example');
...
it('builds each document URL from the site origin and slug', () => {
  const { csv } = toCsv([{ ...ROW, slug: 'edgar-0001' }], 'https://prospectus.example');
