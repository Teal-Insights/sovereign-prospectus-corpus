...
**Date:** 2026-07-19 (v3; council rounds 1 and 2 fully triaged in)
**Status:** Round 1 (Codex gpt-5.6-sol xhigh NOT SOUND; four fresh internal seats, generalist / sovereign-debt domain / consumer / pipeline-seams, all SOUND WITH CHANGES) fully applied in v2. Round 2 (Codex delta verification on v2: 3 round-1 findings CLOSED, 7 PARTIAL, 0 OPEN, verdict NOT SOUND on 4 new CRITICALs + 5 IMPORTANTs inside v2's new mechanisms) fully applied in this v3, which also reconciles the spec with PR #128 (ontology pilots merged to main mid-council). Dispositions in section 18. Round 3 (Codex closure check) next, then Teal sign-off. After sign-off: Stage 2 branch plan, separate session.
...
5. takedown.yml resolves the dedup equivalence class: suppressing a storage key also suppresses documents demoted as its duplicates (the `duplicate_of` chain) and their listings, and the takedown drill proves a canonical/demoted pair cannot leave or resurrect equivalent public content. This build's ingest-side halves of the same contract (never attach to a suppressed document; a hash match against suppressed bytes suppresses the newcomer) are section 5.8's.
...
**The discovery envelope survives the two-phase CLI.** Discovery persists TWO artifacts: the DocRecord JSONL (as today) and a sidecar envelope `<source>_discovery.meta.json` carrying run id, window, mode, outcome, stats, the candidate watermark, and references to any staged source state. Watermark semantics: the adapter receives a window (`since` or the prior token) and returns the candidate in its DiscoveryResult; the runner writes it into the envelope; the platform owns advancement. During discovery, `state_dir` is READ-ONLY except `state_dir/staged/<run-id>/` (Dublin's new directory-snapshot basis is written there, never over the live basis). Promotion is atomic and joint: when the download phase for that envelope completes green (no breaker abort; the end of `corpus download <source>` locally, the state commit under Lane B), the runner advances `data/config/watermarks.json` AND promotes the staged state in one step. An `outcome = partial` envelope never advances the watermark and never promotes (the weekly reconcile covers the gap), so missed pages cannot be skipped permanently and a failed run cannot corrupt the diff basis. Discovery-file format: runner-persisted DocRecord JSONL + envelope supersedes the legacy per-source shapes (NSM raw `_source` dicts, PDIP un-enriched rows); legacy discovery files are void, regenerated by re-running discover (cheap), and the generic download command reads the new format only.
...
| S10: ISIN-disjointness can false-suppress when extraction is partial | Accepted with narrowing; hard suppression only when both sides classify to tranche-level classes (final_terms, supplement); elsewhere disjointness annotates | s5.8, AC 6 |
