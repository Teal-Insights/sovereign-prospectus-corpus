...
**Date:** 2026-07-19 (v2; council round 1 fully triaged in)
**Status:** Council round 1 complete (Codex gpt-5.6-sol xhigh NOT SOUND; four fresh internal seats, generalist / sovereign-debt domain / consumer / pipeline-seams, all SOUND WITH CHANGES). All CRITICALs and convergent IMPORTANTs applied in this v2; dispositions in section 18. Round 2 (Codex delta verification) next, then Teal sign-off. After sign-off: Stage 2 branch plan, separate session.
...
**Division of labor with Lane B, explicit:** this spec owns the contract, the registry, the runner, the dedup pass, the two one-time platform refactors named in section 5.14, and the Dublin adapter. Lane B owns the schedulers, state shuttle, register/health/alarm **evaluation**, and the feeder plumbing. The contract **declares** per-source register fields, health fields, and alarm thresholds; Lane B's builders iterate the registry to evaluate them. Either can land first.
...
2. The state revision artifact list gains `watermarks.json` and `source_state/<source>/` (small per-source artifacts such as Dublin's directory snapshot basis).
...
- **Registry:** `config.toml` gains `[corpus] active_sources = ["nsm", "edgar", "pdip", "luxse", "dublin"]` (order = display order). The registry resolves each name against a configurable package list (default `["corpus.sources"]`; tests prepend a fixture package, which is how toysource exists without shipping in the product package) and validates the protocol at startup. `[pdip_annotations]` and other non-source config sections are untouched; only `active_sources` names are sources.
...
[dublin.cli_options]               # optional extra per-source CLI options the generic commands expose
```
...
Watermark semantics: the adapter receives a window (`since` or the prior token), returns a candidate new watermark in its DiscoveryResult, and the platform owns storage and advancement. Locally, `data/config/watermarks.json` advances when the download phase for that discovery window completes without a breaker abort (the end of a green `corpus download <source>` run); under Lane B, at state commit. Discovery-file format: runner-persisted DocRecord JSONL supersedes the legacy per-source shapes (NSM raw `_source` dicts, PDIP un-enriched rows); legacy discovery files are void, regenerated by re-running discover (cheap), and the generic download command reads the new format only.
...
Per-source register row fields the contract guarantees (Lane B's builder renders them): display name, cadence class, scheduled state, holdings count, known-gap notes (config-declared), quarantine count, dedup attestation count, near-dup pending pairs, review-lane items (each with its stable detected date), ToS pointer. **Holdings-register discipline (platform round-2 seam, honored):** no run ids, no timestamps-of-latest-run, nothing that changes on a no-change day; run ids and operational state live in the health beacon. Review-lane and near-dup entries change the register only when items appear or resolve, which are real changes. Health beacon fields per source (Lane B evaluates): last discovery success, last new document date, latest run outcome. The adapter contributes outcomes through runner telemetry; it never writes the register itself.
...
- **Ingest attach rule:** a new record whose `file_hash` matches an existing document attaches as an `exact-hash` listing on that document (oldest `document_id` wins when several existing rows share the hash, which is real today: clusters up to 11 rows) and mints no document row. The manifest record persists on disk regardless. The listing's `metadata` preserves the newcomer's document-level knowledge (issuer_type, linkage notes); its `countries` merge into `document_countries` (additive inserts in the same transaction; the PK dedups); the canonical document's own fields are never overwritten. Attachments never mint feed entries.
- **Idempotency:** the ingest skip set is `documents.storage_key` UNION `document_listings.storage_key`, so re-reading the same manifest line (routine: ingest rescans manifest dirs) reports `skipped_exists` instead of violating UNIQUE or double-attaching.
...
- **Soft near-dup advisory:** after each ingest batch, candidate pairs are: intra-source, equal normalized issuer (casefold, collapse whitespace) + equal raw `doc_type` + equal non-null `publication_date`; cross-source, equal normalized issuer + equal non-null `publication_date` with the `doc_type` term DROPPED (source vocabularies never string-match, so requiring it made the advisory structurally blind to exactly the cross-venue case that motivates it). Suppression: pairs whose ISIN sets are both non-empty and disjoint are suppressed (same-day multi-series EMTN drawdowns are distinct by construction; empty sets never suppress). Different hashes required, dispositioned pairs excluded. Advisories render in the register section and refresh PR body; never blocking, never auto-suppressing. Known limitation, stated in the register's coverage language: until the vocabulary and issuer-canonicalization pilots land, issuer normalization is crude ("Kenya, Republic of" vs "Republic of Kenya" will NOT match), so the advisory under-detects; byte identity also under-detects same-work cross-venue pairs whenever a venue re-linearizes the PDF. Whether byte-identical cross-venue pairs exist at volume is an empirical output of the backfill, not an assumption.
...
- **Demotion mechanics (corrected in v2):** apply re-points the non-canonical document's existing listing row to the canonical document with `attachment_basis='curated'` (UNIQUE(source, native_id) holds; the demoted document now has zero listings), sets `documents.duplicate_of_document_id` (new nullable column, additive ALTER) on the demoted row, and leaves it `in_scope`. The snapshot emits the row with an additive `duplicate_of` slug column (parquet contract is additive-only), so the page stays live and the URL never breaks; browse hiding and count exclusion happen when Lane A consumes the column (a named Lane A follow-up, not this build). `scope_status='excluded'` is NOT used for this: chair-verified, exclusion means absent from parquet AND the stale-slug sweep deletes the text JSON, and the platform's takedown semantics rely on exclusion meaning exactly that.
- **Curated listings and the divergence edge:** `curated` listings are excluded from byte-divergence logic by basis (their bytes never matched the canonical's). For `exact-hash` listings, divergence monitoring (venue silently replaces its copy) requires the Lane B revalidation extension (handed delta 3); until it lands, the register's limitations note says listing URLs are not re-checked. Page-citation integrity note for whenever a listing link is rendered: a curated listing's venue copy may paginate differently from the canonical text, so any future listing UI labels curated links as a venue variant, never as the cited text.
...
### 5.14 Two one-time platform refactors that make the bar true (this build, not per source)

1. **Snapshot country resolution.** Today the snapshot resolves country by exact `issuer_name` lookup in the hard-coded `ISSUER_TO_COUNTRY` map and ignores `document_countries` entirely (chair-verified at snapshot.py:28/139/287), so any new source's unfamiliar issuer spelling renders "Unknown" until someone edits a platform reference file, which breaks the zero-edit bar and worsens the three-way country-reference drift (issue #80). One-time refactor landing with the contract: the snapshot prefers `document_countries` (issuer-role row) and falls back to the legacy map for rows without one. Existing rows are unaffected (they resolve exactly as today); Dublin rows carry countries from the allowlist through ingest.
...
- **Tier B (legacy shims): nsm, edgar, pdip, luxse.** Each gains a protocol shim exposing `discover`/`fetch`/`source_page` and an IncrementalSpec (`supports_since = false`), with internals grandfathered: they keep their own writes, sleeps, and disk-keyed skips for now. The shim translates legacy statuses to the closed vocabulary at the reporting boundary (`no_pdf_link` to `skipped_no_document`, `failed_invalid_pdf` to `failed_invalid_content`, etc.); the translation table ships in the parity matrix, stats keys unify, manifest FORMAT is unchanged, and the affected assertions in the existing per-source tests are updated deliberately. The contract suite asserts protocol shape, provenance, ToS, registry membership, and boundary status vocabulary for Tier B; runner-execution assertions (no-write, no-sleep, hash-keyed skips) apply to Tier A only.
...
5. **Explorer UI changes** (no browse hiding, no "also listed on" surface; the snapshot gains only additive columns, and Lane A consumes `duplicate_of` later).
6. **Building refresh.yml, the state shuttle, register/health/alarm evaluation, or the feeder plumbing** (Lane B owns them; section 4 hands it four named seam deltas).
...
| 13 | The two 5.14 platform refactors regress existing behavior | Mitigated: country resolution falls back to the legacy map (existing rows resolve identically); filing_url change is a deliberate, called-out data improvement replacing dead signed URLs; both covered by fixture tests |
...
- The two section 5.14 platform refactors merged with fixture coverage; LuxSE mini-spike dispositioned (per-doc scheme found and follow-up minted, or landing-page fallback recorded).
...
- Register/health/alarm declarations in config consumed by whatever Lane B evaluation exists at merge time, or verified available to it by test if Lane B lands later; the four seam deltas of section 4 recorded on Lane B's Stage 2 issue.
...
2. When any adapter's discovery hits a cap or drops a page, then DiscoveryResult states available vs captured and the run log says so explicitly (no silent truncation), and the runner receives the candidate watermark through the same structure.
3. When a fetched document's bytes hash-match an existing document, then no new document row is minted, an `exact-hash` listing attaches to the oldest matching document with its metadata and countries merged transactionally, the manifest record persists, and the register's attestation count increments.
4. When the same manifest lines are re-ingested, then every previously attached record reports `skipped_exists` (skip set spans documents AND listings), nothing double-attaches, and no uniqueness violation occurs.
...
6. When issuer and non-null publication_date match with different hashes (plus equal raw doc_type intra-source), then a near-dup advisory appears in the register section and PR body unless the pair's ISIN sets are both non-empty and disjoint; both documents remain published; dispositioned pairs never re-surface.
7. When `corpus dedup apply` executes a "duplicate" disposition, then the demoted document's listing re-points to the canonical with basis `curated`, `duplicate_of_document_id` is set, the row stays `in_scope`, the next snapshot still emits its row and text (page reachable, URL unchanged) with the additive `duplicate_of` column populated, and re-running apply is a no-op. (Browse hiding: Lane A, when it consumes the column.)
...
15. When a snapshot builds over a document whose `document_countries` has an issuer-role row, then country metadata comes from it; when none exists, the legacy map result is byte-identical to today's (fixture-tested both ways).
