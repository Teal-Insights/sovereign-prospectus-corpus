...
**Status:** Council round 1 (Codex xhigh NOT SOUND, Opus max SOUND WITH CHANGES, Sonnet max SOUND WITH CHANGES) fully triaged into v2; round 2 (Codex xhigh delta verification, NOT SOUND on five new seams in the revised model) fully triaged into v3; round 3 resolution check pending, then Teal sign-off. Dispositions in section 22.
...
Trigger: push to main with `docs/refresh/RUN.json` changed. Concurrency: group `publish`, queued. Permissions include `issues: write`.
...
5. **Activate:** one conditional MANIFEST PUT (If-Match on the journaled ETag). CAS failure aborts with an alarm instead of interleaving.
...
9. When a candidate's suppression epoch is older than the ledger's, then publish refuses it with an alarm and no public mutation occurs.
